...
function AxisBoundLineChart() {
  ...
  'Grammar: Line Chart': `import { VistralChart, registerTheme, type VistralSpec, type ChartHandle, type VistralTheme } from '@timeplus/vistral';
import { dataGenerators } from './data-utils';

// Register a custom theme once at app startup
...
function GrammarCandlestickChart() {
  ...
  'Table (Trend Indicators)': `import { StreamChart, useStreamingData, type StreamDataSource, type TableConfig } from '@timeplus/vistral';

function TrendIndicatorTable() {
  const theme = useTheme();
  const { data, append } = useStreamingData<unknown[]>([], 500);
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => {
      ...
      const change = (Math.random() - 0.48) * prev * 0.02;
